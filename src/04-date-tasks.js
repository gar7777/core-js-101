...
function isLeapYear(date) {
  ...
  if (currentYear % 100 !== 0) return true;
  if (currentYear % 400 !== 0) return false;
  return true;
}
...
function timeSpanToString(startDate, endDate) {
  ...
  const afterHours = diff % 3600000;
  if (hours < 10) {
    hours = `0${hours}`;
  }
  let minutes = Math.floor(afterHours / 60000);
  const afterMinutes = diff % 60000;
  if (minutes < 10) {
    minutes = `0${minutes}`;
  }
  let seconds = Math.floor(afterMinutes / 1000);
